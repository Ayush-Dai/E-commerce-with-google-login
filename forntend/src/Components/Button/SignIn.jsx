    ...
    return (
        <>
            ...
            <div className="flex justify-center flex-col items-center h-[80vh] ">
                <form className="w-[600px] p-6 mx-auto rounded-lg bg-gray-600/100 shadow-md" onSubmit={handleSubmit}>


                    <label className="block mb-2 text-white">
                        Email:
                        <input type="email" name="email" value={values.email} className="block w-full border p-2 rounded mt-1" onChange={handleChange} onBlur={handleBlur} required />
                        {errors.email && touched.email && <p className='text-red-400'>{errors.email}</p>}
                    </label>

                    <label className="block mb-2 text-white">
                        Password:
                        <input type="password" name="password" value={values.password} className="block w-full border p-2 rounded mt-1" onChange={handleChange} onBlur={handleBlur} required />
                        {errors.password && touched.password && <p className='text-red-400'>{errors.password}</p>}
                    </label>

                    <button type="submit" className="mt-2 hover:bg-green-700 cursor-pointer bg-green-500 text-white px-4 py-2 rounded">
                        Log in
                    </button>

                    <p className="inline pl-2.5 text-white">
                        Doesn't have an account? <a className="text-blue-300 hover:text-blue-500" href="/signup">Sign Up</a>
                    </p>
                </form>
                <p>or</p>
